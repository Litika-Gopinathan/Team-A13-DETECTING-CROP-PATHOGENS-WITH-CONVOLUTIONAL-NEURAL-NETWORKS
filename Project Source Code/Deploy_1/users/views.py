...
def model(request):
    ...
    if request.method == "POST":
        ...
        models = keras.models.load_model('C:/FinalProject/Project/Deploy_1/users/model.h5')
        data = np.ndarray(shape=(1, 224, 224, 3), dtype=np.float32)
        
        image = Image.open('C:/FinalProject/Project/Deploy_1/media/'+ str(result1)).convert("RGB")
        size = (224, 224)
        image = ImageOps.fit(image, size, Image.ANTIALIAS)
        image_array = np.asarray(image)
        normalized_image_array = (image_array.astype(np.float32) / 127.0) - 1
        data[0] = normalized_image_array
        classes = ["Anthracnose","Apple_Scab","Botrytis","Brown_Rot","Cedar_Apple_Rust","Crown_Gall",
                   "Downy_Mildew","Fire_Blight","Leaf_Spots","Mosaic_Virus","Powdery_Mildew","Verticillium"]
